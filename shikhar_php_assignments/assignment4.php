        ...
        <form name="login_form" id="login_form" action=<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?> class="form-horizontal" method="post">
            <div class="container">
                <div class="form-group">
                    ...
                    <div class="col-sm-5">
                        <input type="text" class="form-control" name="user_text" id="user_text" required="required" />
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-sm-4" for="user_password">Password</label>
                    <div class="col-sm-5">
                        <input type="password" class="form-control" name="user_password" id="user_password" required="required" />
                    </div>
                </div>
                <div class="form-group">
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                if(isset($_SESSION['student'])){
                    for($i = 0; $i < count($_SESSION['student']); $i++) {
                        if((strtolower($_SESSION['student'][$i]['name_text']) == strtolower($_POST['user_text'])) && ($_SESSION['student'][$i]['user_password'] == $_POST[user_password])) {
                            $_SESSION['login'] = 'true';
